...
전국 69개 지역신문과 19개 종합지에서 최근 기사를 모으고, 20·30대와 청년계층이 겪는 구체적인 고충과 어려움을 매일 Markdown·HTML 리포트로 정리하는 파일 기반 자동화입니다.
...
```mermaid
flowchart TD
    A["전국 88개 신문사 목록"] --> B["1단계: 기사 수집 로봇<br/>매일 아침 6시 자동 실행"]
    B --> C["신문사 사이트에서<br/>최신 기사 목록 가져오기"]
    C --> D["최근 하루치 기사 중<br/>청년·고충 단어가 함께 든 기사만 추리기"]
    ...
    K -- 예 --> M["제목과 요약을 읽고<br/>실제 청년 고충 기사인지 O/X 판별"]
...
- `data/candidates-YYYY-MM-DD.json`: 최근 기사 중 청년·2030 대상어와 고충 신호를 함께 포함한 1차 후보와 수집 통계
...
주제나 신문사 계약을 바꾼 뒤에는 과거 후보 JSON을 새 계약으로 재해석하지 않습니다. 변경을 반영한 직후 **Collect article candidates**를 수동 실행해 당일 후보를 새로 만든 다음 리포트 작업을 실행하세요.

## 내 주제에 맞게 바꾸기

청년·고충 키워드, 최종 판별 기준, 신문사 목록을 바꾸려면 [CUSTOMIZE.md](CUSTOMIZE.md)를 참고하세요. 날짜·준비 상태·출처 SHA·멱등성 규칙은 운영 안전장치이므로 변경하지 마세요.
